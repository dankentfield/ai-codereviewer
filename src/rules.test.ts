...
import { getApplicableRules, readRules, RulesFile } from "./rules";
  ...
  test("should read and return rules from the file", () => {
    ...
    expect(rules).toEqual({
      directories: [
        {
          paths: ["*Consumer.ex"],
          rules: ["Update catalog-info.yaml", "Add schema to events schemas"],
        },
        {
          paths: ["*Publisher.ex"],
          rules: ["Update catalog-info.yaml", "Add schema to events schemas"],
        ...
        {
          paths: ["**/context/**"],
          rules: ["Include comprehensive tests"],
        },
        {
          paths: ["**/resolvers/**"],
          rules: ["Use GraphQL fragments for leaf components"],
        },
        {
          paths: ["**/scripts/**"],
          rules: [
        ...
        {
          paths: ["**/e2e/**"],
          rules: ["New user flows must have e2e test coverage"],
        },
      ],
      extensions: [
        {
...
describe("getApplicableRules", () => {
  ...
  test("Given an ignore file, it should return only global rules", () => {
    ...
    expect(result).toEqual(["Always be concise"]);
  });
